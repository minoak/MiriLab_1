...
def build_village_context(villagers, locations):
    ...
    return (
        ...
        "- 하루 중 한두 번은 공용 공간(분수광장·마을공원·마을텃밭·마을회관·복지관·"
        "어린이놀이터·카페 등)에 들러 이웃과 어울리는 시간 블록을 넣는다(집·직장에만 머물지 말 것).\n"
        "- 단 모두가 카페로 몰리지 않도록, 인물의 성격·동선·생활 리듬에 맞는 서로 다른 공용 "
        "공간으로 분산한다. 카페는 그 인물에게 자연스러울 때만(점심·일과 관련 등) 고른다.\n"
