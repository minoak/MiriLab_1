...
def _personas() -> list:
    ...
    raw = [
        ...
        ("p13", "배지민", "22세 여성 · 미혼 · 대학생 · 대구 북구 원룸",
         "여성", 22, "미혼", "1인 가구", "월세(원룸)", "대학교 재학", "대학생",
         "북구", "대구광역시",
         0.93, "저소득", 0.44, ["과 동기 단톡방", "에브리타임", "가족 카톡방"],
         "타지에서 학교를 다니며 아르바이트로 생활비를 보탠다. 월세 고지서가 올 때마다 "
         "다음 달 생활비를 다시 계산한다."),

        ("p14", "나현우", "30세 남성 · 미혼 · 프리랜서 · 서울 마포구 반전세",
         "남성", 30, "미혼", "1인 가구", "반전세(오피스텔)", "대학교 졸업", "프리랜서 디자이너",
         "마포구", "서울특별시",
         0.88, "중간소득", 0.37, ["작업 단톡방", "디자인 커뮤니티"],
         "수입이 달마다 달라서 소득 증빙을 할 때마다 부담을 느낀다. 정책이 있어도 "
         "서류가 복잡하면 신청을 미루는 편이다."),

        ("p15", "문정아", "27세 여성 · 미혼 · 계약직 · 인천 남동구 월세",
         "여성", 27, "미혼", "1인 가구", "월세(빌라)", "전문대 졸업", "계약직 사무보조",
         "남동구", "인천광역시",
         0.74, "저소득", 0.42, ["회사 동료", "동네 친구"],
         "계약직으로 일하며 월급 대부분이 생활비와 월세로 나간다. 지원 기준을 알기 쉽게 "
         "보여주면 바로 신청할 의향이 있다."),

        ("p16", "최광호", "66세 남성 · 기혼 · 은퇴 · 부산 사하구",
         "남성", 66, "기혼", "부부", "자가(아파트)", "고등학교 졸업", "은퇴",
         "사하구", "부산광역시",
         0.22, "저소득", 0.62, ["노인복지관", "동네 산악회"],
         "은퇴 후 아내와 생활비를 아껴 쓰며 지낸다. 청년 정책이라도 손주에게 도움이 되는지 "
         "알아보고 싶지만 온라인 안내는 오래 보기 어렵다."),

        ("p17", "서민정", "38세 여성 · 이혼 · 돌봄노동 · 광주 서구 월세",
         "여성", 38, "이혼", "한부모+자녀", "월세(다세대)", "고등학교 졸업", "요양보호사",
         "서구", "광주광역시",
         0.58, "저소득", 0.40, ["한부모 모임", "직장 동료"],
         "아이를 혼자 키우며 야간 근무도 한다. 청년 기준에서 빠지더라도 월세 부담이 큰 "
         "가정이 함께 안내받을 수 있기를 바란다."),

        ("p18", "유하늘", "34세 남성 · 기혼 · 회사원 · 대전 유성구 월세",
         "남성", 34, "기혼", "신혼부부(무자녀)", "월세(아파트)", "대학교 졸업", "회사원",
         "유성구", "대전광역시",
         0.82, "중간소득", 0.49, ["회사 동기", "신혼부부 카페"],
         "결혼 초기라 주거비가 가장 큰 부담이다. 부부 가구도 청년 월세 지원에서 어떻게 "
         "판정되는지 확인하고 싶어 한다."),

        ("p19", "장은비", "25세 여성 · 미혼 · 취업준비생 · 서울 동작구 고시원",
         "여성", 25, "미혼", "1인 가구", "고시원", "대학교 졸업", "무직(취준생)",
         "동작구", "서울특별시",
         0.86, "저소득", 0.36, ["취준 스터디", "온라인 커뮤니티"],
         "취업 준비를 하며 고시원비를 아끼려고 식비를 줄인다. 고시원 계약서가 지원 서류로 "
         "인정되는지가 가장 걱정이다."),

        ("p20", "홍성민", "41세 남성 · 기혼 · 택배기사 · 경기 안산시 월세",
         "남성", 41, "기혼", "부부+자녀", "월세(빌라)", "고등학교 졸업", "택배기사",
         "안산시", "경기도",
         0.52, "중간소득", 0.34, ["동료 기사", "가족방"],
         "일이 불규칙해 주민센터 방문 시간을 내기 어렵다. 청년이 아니어서 직접 혜택은 없지만 "
         "기준이 너무 좁다고 느낀다."),

        ("p21", "김태윤", "20세 남성 · 미혼 · 군 전역 예정 · 충북 청주시",
         "남성", 20, "미혼", "부모와 동거", "부모 자가", "고등학교 졸업", "군 전역 예정",
         "청주시", "충청북도",
         0.80, "저소득", 0.46, ["고향 친구", "가족 카톡방"],
         "전역 후 독립을 준비하고 있다. 아직 부모 집에 살아서 신청할 수 있는지와 독립 후 "
         "언제부터 가능한지가 궁금하다."),

        ("p22", "오세라", "32세 여성 · 미혼 · 간호조무사 · 전북 전주시 원룸",
         "여성", 32, "미혼", "1인 가구", "월세(원룸)", "전문대 졸업", "간호조무사",
         "전주시", "전라북도",
         0.69, "저소득", 0.52, ["직장 단톡방", "동네 친구"],
         "교대 근무를 하며 혼자 산다. 신청 시간이 짧거나 안내가 복잡하면 놓칠까 봐 "
         "체크리스트가 있기를 바란다."),

        ("p23", "신동혁", "29세 남성 · 미혼 · 스타트업 개발자 · 서울 성동구 전세",
         "남성", 29, "미혼", "1인 가구", "전세(오피스텔)", "대학교 졸업", "스타트업 개발자",
         "성동구", "서울특별시",
         0.97, "고소득", 0.41, ["개발자 커뮤니티", "회사 슬랙"],
         "소득 기준 때문에 본인은 제외될 가능성이 크다. 그래도 월세와 전세 사이의 형평성을 "
         "온라인에서 자주 따져 보는 편이다."),

        ("p24", "박은영", "58세 여성 · 기혼 · 식당 자영업 · 강원 원주시",
         "여성", 58, "기혼", "부부", "자가(단독)", "중학교 졸업", "식당 자영업",
         "원주시", "강원특별자치도",
         0.36, "저소득", 0.45, ["시장 상인회", "가족방"],
         "작은 식당을 운영하며 경기가 나쁠 때마다 임대료와 인건비를 걱정한다. 청년만 "
         "지원한다는 말에는 아쉬움을 느낀다."),

        ("p25", "이가은", "23세 여성 · 미혼 · 카페 아르바이트 · 제주 제주시 원룸",
         "여성", 23, "미혼", "1인 가구", "월세(원룸)", "대학교 휴학", "카페 아르바이트",
         "제주시", "제주특별자치도",
         0.84, "저소득", 0.47, ["알바 단톡방", "친구 모임"],
         "휴학 후 아르바이트로 생활비를 벌고 있다. 월세 지원이 되면 바로 신청하고 싶지만 "
         "소득 기준을 계산하는 법을 잘 모른다."),

        ("p26", "정우진", "36세 남성 · 미혼 · 제조 계약직 · 경남 창원시 월세",
         "남성", 36, "미혼", "1인 가구", "월세(원룸)", "고등학교 졸업", "제조 계약직",
         "창원시", "경상남도",
         0.61, "중간소득", 0.33, ["공장 동료", "고향 친구"],
         "나이 기준에서 조금 벗어나 지원을 놓친 경험이 있다. 한두 살 차이로 빠지는 사람을 "
         "위한 안내가 필요하다고 생각한다."),

        ("p27", "차미숙", "70세 여성 · 사별 · 무직 · 충남 공주시",
         "여성", 70, "사별", "1인 가구", "자가(노후 주택)", "초등학교 졸업", "무직",
         "공주시", "충청남도",
         0.12, "저소득", 0.66, ["경로당", "복지관"],
         "혼자 지내며 손주 소식을 카톡으로 받는다. 청년 월세 정책을 보면 손주가 받을 수 "
         "있는지 궁금하지만 글자가 많으면 읽기 어렵다."),

        ("p28", "강서윤", "31세 여성 · 기혼 · 회계 사무직 · 경기 화성시 월세",
         "여성", 31, "기혼", "신혼부부(무자녀)", "월세(아파트)", "대학교 졸업", "회계 사무직",
         "화성시", "경기도",
         0.79, "중간소득", 0.50, ["회사 동료", "신혼 카페"],
         "신혼집 월세를 내며 출산 계획을 고민한다. 청년 가구 기준에서 부부 소득과 원가구 "
         "기준이 어떻게 적용되는지가 가장 궁금하다."),

        ("p29", "한재민", "24세 남성 · 미혼 · 대학원생 · 세종시 기숙사",
         "남성", 24, "미혼", "부모와 별거(기숙사)", "기숙사", "대학원 재학", "대학원생",
         "세종시", "세종특별자치시",
         0.91, "저소득", 0.43, ["연구실 단톡방", "학교 커뮤니티"],
         "장학금과 조교비로 생활한다. 기숙사 거주가 월세 지원 대상인지 애매해 보여서 "
         "학교 커뮤니티에서 정보를 확인한다."),

        ("p30", "권도윤", "33세 남성 · 미혼 · 배달노동자 · 서울 영등포구 고시원",
         "남성", 33, "미혼", "1인 가구", "고시원", "고등학교 졸업", "배달노동자",
         "영등포구", "서울특별시",
         0.57, "저소득", 0.31, ["배달 기사 단톡방", "인력사무소 지인"],
         "배달 일을 하며 고시원에 산다. 소득이 일정하지 않고 계약서도 간단해서 신청 서류를 "
         "준비하다가 막힐까 봐 걱정한다."),
    ]
...
def _reaction_for_persona(persona: dict) -> dict:
    """확장 mock 페르소나의 정책 반응을 결정론적으로 만든다."""
    demo = persona.get("demographics") or {}
    sig = persona.get("signals") or {}
    pid = persona.get("id")
    name = persona.get("name") or "시민"
    ...
    network_size = len(sig.get("social_network") or [])
    category = _eligibility_category(age, income, family_type, housing)

    share = min(95, 25 + network_size * 14 + int(dl * 22))
    understand = min(95, 34 + int(dl * 48))

    if category == "eligible":
        stance = "support"
        benefit, intent, dissatisfaction = 82, 86, 14
        if "고시원" in housing:
            text = f"{name} 씨는 고시원비도 월세로 인정되는지만 확인되면 바로 신청하려고 합니다."
            actions = ["서류 확인", "신청 시도", "주변 공유"]
        elif "부부" in family_type or "신혼" in family_type:
            text = f"{name} 씨는 월세 부담이 커서 반갑지만, 부부 소득과 원가구 기준이 어떻게 적용되는지 먼저 보려고 합니다."
            actions = ["주민센터 문의", "신청 시도"]
            stance = "mixed"
            benefit, intent, dissatisfaction = 72, 68, 28
        elif "프리랜서" in occupation or "배달" in occupation:
            text = f"{name} 씨는 도움이 될 것 같지만 불규칙한 소득을 어떻게 증빙하는지가 가장 걱정입니다."
            actions = ["소득 증빙 확인", "신청 보류"]
            stance = "mixed"
            benefit, intent, dissatisfaction = 70, 62, 35
        else:
            text = f"{name} 씨는 월세 부담이 줄어들 수 있어 조건만 맞으면 바로 신청해보려 합니다."
            actions = ["복지로 검색", "신청 시도", "친구 공유"]
    elif category == "parent_block":
        stance = "oppose"
        benefit, intent, dissatisfaction = 18, 14, 72
        text = f"{name} 씨는 청년인데도 부모와 산다는 이유로 빠질 수 있어 기준이 너무 좁다고 느낍니다."
        actions = ["조건 확인", "불만 토로"]
    elif category == "pending":
        stance = "mixed"
        benefit, intent, dissatisfaction = 45, 38, 36
        text = f"{name} 씨는 기숙사나 독립 준비 단계도 인정되는지 몰라 일단 학교와 주민센터에 물어보려 합니다."
        actions = ["학교 문의", "주민센터 문의"]
    else:
        if age >= 60:
            stance = "mixed"
            benefit, intent, dissatisfaction = 12, 8, 30
            text = f"{name} 씨는 본인 대상은 아니지만 자녀나 손주가 받을 수 있는지 쉽게 설명해주면 좋겠다고 봅니다."
            actions = ["가족에게 문의", "주민센터 문의"]
        elif str(income).strip() in ("고소득", "high"):
            stance = "oppose"
            benefit, intent, dissatisfaction = 18, 12, 58
            text = f"{name} 씨는 소득 기준으로 빠질 가능성이 커서 월세 부담과 소득 기준을 더 세밀하게 봐야 한다고 말합니다."
            actions = ["온라인 의견 작성", "기준 확인"]
        else:
            stance = "oppose"
            benefit, intent, dissatisfaction = 20, 12, 66
            text = f"{name} 씨는 월세 부담은 비슷한데 나이 기준 때문에 빠지는 사람이 많아 보완이 필요하다고 봅니다."
            actions = ["불만 토로", "대체 지원 검색"]

    if dl < 0.35:
        understand = min(understand, 38)
        share = max(15, share - 8)
        if "방문" not in text and "주민센터" not in text:
            text += " 온라인만으로는 어렵고 방문 안내가 같이 있어야 합니다."
...
def sample_simstate(policy: str | None = None, n: int = 12) -> dict:
    ...
    Args:
        ...
        n: 페르소나 수. 발표용 mock 풀에서 앞에서부터 n명을 사용(최대 30).
    ...
    # n 명만 사용(최대 mock 풀 크기). 페르소나/반응을 id 기준으로 일관되게 잘라낸다.
    ...
    reaction_ids = {r["persona_id"] for r in all_reactions}
    all_reactions.extend(
        _reaction_for_persona(p)
        for p in all_personas
        if p["id"] not in reaction_ids
    )
