...
반환 키
    ...
    casting           : dict  일탈 행동 캐스팅(DESIGN §9) {threshold, members}. 없으면 {}
"""
...
def _compute_metrics(reactions: list) -> dict:
    ...
    # 일탈 행동 분포(DESIGN §9) — 집계노드(graph.nodes)와 동일 키/규칙.
    # ''(미측정)은 분모에서 제외. sim metrics 가 있으면 _merge_metrics 에서 덮인다.
    behavior_counts: dict = {}
    for r in reactions:
        bc = str(_as_dict(r).get("behavior_class") or "").strip()
        if bc:
...
def build_view(sim) -> dict:
    ...
    for r in raw_reactions:
        ...
        # 일탈 행동 축(DESIGN §9) — 항상 str 로 보정('' = 미측정/없음).
        fixed["behavior_class"] = str(rd.get("behavior_class") or "")
        fixed["behavior_tag"] = str(rd.get("behavior_tag") or "")
        fixed["behavior_text"] = str(rd.get("behavior_text") or "")
    ...
    # --- 일탈 행동 캐스팅(DESIGN §9): 있으면 그대로 통과('왜 이 사람이' 근거) ---
    casting = _as_dict(s.get("casting"))

    # --- 생성 모델 도장(측정 도구 표식, run_full_pipeline 이 찍음): 통과만 ---
    llm_model = str(s.get("llm_model") or "")
