...
- get_personas(n, seed, filters_json) : 페르소나 리스트 — @st.cache_resource
...
@st.cache_resource(show_spinner=False)
def get_personas(n: int = 24, seed: int = 42, filters_json: str = ""):
    """페르소나 리스트를 캐시한다.

    첫 호출에서만 HF 데이터셋 다운로드/샘플링이 일어나고,
    이후 동일 (n, seed, filters) 조합은 캐시된 결과를 즉시 반환한다.

    filters 는 dict 대신 JSON 문자열로 받는다 — st.cache_resource 인자 해시가
    Streamlit 버전과 무관하게 안정되도록(버전 드리프트 방지). "" = 필터 없음.
    """
    filters = json.loads(filters_json) if filters_json else None
    return load_personas(n, seed, filters=filters)


def _filters_json(filters: dict | None) -> str:
    """필터 dict -> get_personas 용 안정 직렬화(키 정렬). None/빈 dict -> ""."""
    return json.dumps(filters, ensure_ascii=False, sort_keys=True) if filters else ""
...
def run_simulation(
    ...
    filters: 시민 구성 조건(data.personas.load_personas 참고). 실모드에서만
    의미가 있다 — 데모(합성 mock)는 자체 생성이라 적용되지 않는다.
    ...
    try:
        with st.status("시뮬레이션 준비 중...", expanded=False) as status:
            ...
            personas = get_personas(n, seed, _filters_json(filters))
...
def run_full_pipeline(
    ...
    Args:
        ...
        mock/n/seed/grounded/rounds/filters: run_simulation 과 동일. mock 은
            축1·2 공통(데모 체크 시 두 축 모두 외부 호출 0 — 출처 일관).
            filters(시민 구성)는 축1 페르소나 추첨에만 관여 — 축2·3은 축1이
            모은 시민을 그대로 쓴다.
    ...
    ckpt["axis1"] = {
        "sig": {"policy": model_policy, "n": n, "seed": seed, "mock": mock,
                "grounded": grounded, "rounds": rounds, "filters": filters},
        "sim": sim,
    }
